...
namespace FileDBReader_Tests
{
    static class TestDataSources
    {
        ...
        internal static BBDocument BuildDocument()
        {
            var filedb = new BBDocument();
            //lets create a few tags.
            Tag root1 = filedb.CreateTag("TestRootOne");

            var Attr = filedb.CreateAttrib("FloatAttrib_Child");
            Attr.Content = BitConverter.GetBytes(33.2f);
            root1.AddChild(Attr);

            var child1 = filedb.CreateTag("None");
            root1.AddChild(child1);

            var root2 = filedb.CreateAttrib("None");
            root2.Content = new UnicodeEncoding().GetBytes("Modders are gonna take over the world");

            var root3 = filedb.CreateAttrib("StringAttrib_Root");
